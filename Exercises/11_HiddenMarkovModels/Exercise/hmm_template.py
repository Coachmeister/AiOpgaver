...
def compute_forward(states, observations, transitions, emissions):
    N = len(states) - 2  # number of states HOT/COLD
    T = len(observations) - 1  # number of observations

    # Set to PI to represent 'empty' values (0 values is valid)
    forward = np.pi * np.ones((N + 2, T + 1))
        ...
        for s in inclusive_range(1, N):
            forward[s, t] = np.array(
                [forward[s_mark, t - 1] * transitions[s_mark, s] * emissions[s, observations[t]] for s_mark in
                 inclusive_range(1, N)]).sum()
            print(forward)
            print("")
    probability = np.array([forward[s, T] * transitions[s, N + 1] for s in inclusive_range(1, N)]).sum()
    return probability
